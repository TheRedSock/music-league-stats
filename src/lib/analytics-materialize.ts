...
export async function getAllLeaguesMaterializationStatus(
  ...
  return job
    ? { analyticsRevision: ANALYTICS_REVISION, job, status: job.status }
    : { analyticsRevision: ANALYTICS_REVISION, job: null, status: "missing" };
}
...
async function markMaterializationJob(
  ...
  update: {
    ...
    summary?: AnalyticsMaterializationSummary | null;
  },
...
/**
 * Rebuild all-leagues materializations.
 * Claim/job bookkeeping and table rebuild use separate transactions so a failed
 * rebuild rolls back deletes/inserts while still recording a failed job.
 */
export async function refreshAllLeaguesMaterialization(
  ...
  const claim = await database.transaction(async (tx) => {
    ...
    if (!force && existing?.status === "completed") {
      return { job: existing, shouldRebuild: false as const };
    }
    ...
    const [job] = await tx
      ...
      .returning();
    return { job, shouldRebuild: true as const };
  });

  if (!claim.shouldRebuild) return claim.job;

  try {
    await database.transaction(async (tx) => {
      await tx.execute(sql`select pg_advisory_xact_lock(${MATERIALIZATION_LOCK_KEY})`);
      await rebuildMaterializedTables(tx);
    });
    ...
    const summary: AnalyticsMaterializationSummary = {
      ...
      relationshipMutual: await countRows(database, "analytics_relationship_mutual"),
      relationshipPairs: await countRows(database, "analytics_relationship_pairs"),
      songStats: await countRows(database, "analytics_song_stats"),
    };

    const completed = await markMaterializationJob(database, claim.job.id, {
      errorMessage: null,
      status: "completed",
      summary,
    });
    ...
    const failed = await markMaterializationJob(database, claim.job.id, {
      errorMessage: message,
      status: "failed",
    });
    try {
      // Drop cached all-leagues mat responses so readers re-check readiness
      // and fall back to live SQL after a failed refresh.
      await revalidateAfterMaterialization();
...
async function rebuildMaterializedTables(tx: SqlExecutor): Promise<void> {
